...
class BuddySupabaseUser extends BaseAuthUser {
  BuddySupabaseUser(this.user);
  User? user;
  @override
...
Stream<BaseAuthUser> buddySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BuddySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
